...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("starcode-relay-traces", "prod")).toBe(
      "starcode-relay-traces-prod",
    );
    expect(relayResourceNameForStage("starcode-relay-traces", "dev_julius")).toBe(
      "starcode-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "starcoderelay.com", hash)).toBe(
      "prod-abcdef0123456789.starcoderelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "starcoderelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
