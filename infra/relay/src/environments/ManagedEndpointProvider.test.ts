...
const config = RelayConfiguration.RelayConfiguration.of({
  ...
  apns: {
    ...
    bundleId: "com.starcode.starcode.dev",
  },
  apnsDeliveryJobSigningSecret: Redacted.make("job-secret"),
  clerkSecretKey: Redacted.make("clerk-secret"),
  clerkPublishableKey: "pk_test_test",
  clerkJwtAudience: "starcode-relay",
  cloudMintPrivateKey: Redacted.make("cloud-private-key"),
  cloudMintPublicKey: "cloud-public-key",
  managedEndpointBaseDomain: "starcode.test",
...
function expectedManagedHostname(environmentId: string, userId = "user_ABC"): string {
  const hash = NodeCrypto.createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 16);
  return `dev-julius-${hash}.starcode.test`;
}
...
function expectedManagedTunnelName(environmentId: string, userId = "user_ABC"): string {
  const hash = NodeCrypto.createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 16);
  return `starcoderelay-managedendpoint-dev-julius-${hash}`;
}
...
describe("ManagedEndpointProvider", () => {
  ...
  it.effect("uses stage-scoped stable names without leaking unusual environment ids", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(requestedName).toMatch(/^starcoderelay-managedendpoint-dev-julius-[a-f0-9]{16}$/);
      const configBody = (
      ...
      expect(configBody).toMatchObject({
        ingress: [
          {
            hostname: expect.stringMatching(/^dev-julius-[a-f0-9]{16}\.starcode\.test$/),
          },
          { service: "http_status:404" },
        ],
      });
