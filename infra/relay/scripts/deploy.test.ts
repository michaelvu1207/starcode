...
describe("reconcileRootEnvRelayUrl", () => {
  ...
  it("preserves unrelated root env entries while replacing a previous relay URL", () => {
    expect(
      reconcileRootEnvRelayUrl(
        "STARCODE_CLERK_PUBLISHABLE_KEY=pk_test_example\nSTARCODE_RELAY_URL=https://old.example.test\n",
...
describe("reconcileRootEnvPublicConfig", () => {
  const config = {
    ...
    mobileTracingUrl: "https://api.axiom.co/v1/traces",
    mobileTracingDataset: "starcode-mobile-traces-dev",
    mobileTracingToken: "xaat-public-ingest",
    clientTracingUrl: "https://api.axiom.co/v1/traces",
    clientTracingDataset: "starcode-relay-client-traces-dev",
  ...
  it("adds the complete local client config", () => {
    expect(reconcileRootEnvPublicConfig("", config)).toBe(
      [
        ...
        "STARCODE_MOBILE_OTLP_TRACES_DATASET=starcode-mobile-traces-dev",
        "STARCODE_MOBILE_OTLP_TRACES_TOKEN=xaat-public-ingest",
        "STARCODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "STARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET=starcode-relay-client-traces-dev",
        "STARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=xaat-relay-client-ingest",
        "",
      ].join("\n"),
    );
  });
    ...
    expect(
      reconcileRootEnvPublicConfig(
        [
          "STARCODE_CLERK_PUBLISHABLE_KEY=pk_test_example",
          "STARCODE_RELAY_URL=https://old.example.test",
          "STARCODE_MOBILE_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "STARCODE_MOBILE_OTLP_TRACES_DATASET=old-dataset",
          "STARCODE_MOBILE_OTLP_TRACES_TOKEN=old-token",
          "STARCODE_RELAY_CLIENT_OTLP_TRACES_URL=https://old.example.test/v1/traces",
          "STARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET=old-client-dataset",
          "STARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=old-client-token",
          "",
        ].join("\n"),
        config,
      ),
...
describe("serializeRelayClientTracingEnvironment", () => {
  it("serializes tracing config for downstream GITHUB_ENV loading", () => {
    expect(
      ...
      [
        "STARCODE_RELAY_CLIENT_OTLP_TRACES_URL=https://api.axiom.co/v1/traces",
        "STARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET=relay",
        "STARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN=client-token",
        "",
      ].join("\n"),
    );
  });
