...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    STARCODE_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    STARCODE_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    STARCODE_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    STARCODE_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    STARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    STARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("STARCODE_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("STARCODE_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "StarcodeRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the starcode relay through Alchemy."));
