# starcode Connect Relay

> [!WARNING]
> starcode Connect is currently in private beta. Join the waitlist in the app under Settings > starcode Connect.

The relay is the hosted control plane for starcode Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal starcode traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [starcode Connect architecture overview](../../docs/cloud/starcode-connect-auth-flow.html) for the larger system
design.
...
- Linking starcode environments to a cloud account.
...
```sh
vp run --filter starcode-relay deploy
```
...
```sh
vp run --filter starcode-relay deploy -- --stage prod
vp run --filter starcode-relay deploy -- --env-file .env.local
```
...
- [starcode Connect Clerk Setup](../../docs/cloud/starcode-connect-clerk.md) for Clerk keys, JWT templates, and waitlist
  setup.
- [Relay Observability](../../docs/relay-observability.md) for deployment tracing and diagnostics.
- [starcode Connect Architecture Overview](../../docs/cloud/starcode-connect-auth-flow.html) for the full link,
