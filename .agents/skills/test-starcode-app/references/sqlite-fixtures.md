...
Load this reference only when inspecting or seeding local starcode state directly.
...
```bash
node apps/server/scripts/starcode-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/starcode-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/starcode-seed.sql
```
