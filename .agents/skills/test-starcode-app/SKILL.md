---
name: test-starcode-app
description: Launch, retain, and test the starcode web app in isolated development environments, including first-try browser authentication with one-time pairing URLs, pairing-token recovery, worktree-safe state directories, cross-turn dev server lifecycle, and direct SQLite inspection or fixture seeding. Use when an agent needs to run starcode locally, iteratively test UI behavior with a human, recover from an expired or consumed pairing token, isolate dev state, or prepare test data in state.sqlite.
---

# Test starcode App

Use this skill for the web client. For iOS Simulator, Android Emulator, or physical-device testing against an isolated starcode backend, use the sibling [`test-starcode-mobile`](../test-starcode-mobile/SKILL.md) skill.

## Start an isolated web environment

1. Run commands from the repository root.
2. Choose a base directory that belongs only to the current worktree or test:
   - Use the repository's ignored `.starcode` directory for reusable worktree-local state.
   - Use `mktemp -d /tmp/starcode-test.XXXXXX` for disposable state and retain the printed absolute path.
...
Treat a base directory as disposable only when it was created or deliberately selected for the current test. Never delete or directly seed the shared `~/.starcode` directory. Prefer starting with a new temporary base directory over clearing state of uncertain ownership.
...
Use the `Pair URL` from this command once. Derive `<server-port>` and `<web-url>` from the current dev-runner output, including any automatically selected port offset. Setting `STARCODE_PORT` keeps the administrative CLI from probing for an unrelated free port.
...
- Use `node apps/server/scripts/starcode-sqlite-state.ts query` for schema discovery and read-only checks.
- Stop the dev server before using `node apps/server/scripts/starcode-sqlite-state.ts exec`, then restart it with the same base directory.
...
The helper refuses to write to the shared `~/.starcode` directory by default and creates a database backup before each mutation.
