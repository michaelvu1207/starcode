---
name: test-starcode-mobile
description: Launch and test starcode Mobile on an iOS Simulator or Android Emulator against disposable local starcode environments, including Metro and dev-client reuse, native rebuild decisions, per-client pairing, seeded projects, semantic UI control, screenshots, and iOS serve-sim streaming. Use after mobile UI or native changes, when reproducing phone or tablet behavior, pairing an emulator to isolated state, or verifying mobile behavior on macOS, Linux, or Windows.
---

# Test starcode Mobile

Run one focused, end-to-end mobile verification pass against disposable starcode state. Use the sibling [`test-starcode-app`](../test-starcode-app/SKILL.md) skill as the detailed reference for pairing-token semantics and SQLite fixtures.
...
- App: `starcode Dev`
- Bundle/package identifier: `com.starcode.starcode.dev`
- URL scheme: `starcode-dev`
...
## Start one disposable starcode environment

Run backend commands from the repository root. Use the ignored, worktree-local `.starcode` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.starcode` state.
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-starcode-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `starcode-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/StarcodeDev.xcworkspace`
- Scheme: `StarcodeDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.starcode.starcode.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.starcode.starcode.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.starcode.starcode.dev
...
```bash
STARCODE_PORT=<server-port> node apps/server/src/bin.ts auth pairing create \
  --base-dir <base-dir> \
...
In PowerShell, set `$env:STARCODE_PORT = "<server-port>"` first and run the `node ... auth pairing create` command without the leading assignment.
...
```bash
xcrun simctl openurl <simulator-udid> 'starcode-dev://connections/new'
adb -s <emulator-serial> shell am start -W \
  -a android.intent.action.VIEW \
  -d 'starcode-dev://connections/new' \
  com.starcode.starcode.dev
```

Run only the command for the selected platform.

In starcode Dev, open Add Environment and enter the complete `<mobile-origin>` and newly printed `Token`. Verify the expected seeded projects appear before exercising the affected flow.
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in starcode when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from starcode Dev.
