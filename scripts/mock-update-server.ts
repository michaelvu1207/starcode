...
const resolveMockUpdateServerConfig = Effect.gen(function* () {
  ...
  const config = yield* Config.all({
    port: Config.port("STARCODE_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.withDefault(3000)),
    root: Config.string("STARCODE_DESKTOP_MOCK_UPDATE_SERVER_ROOT").pipe(
