...
it("selects an explicit CI Android ABI without changing the local default", () => {
  ...
  assert.throws(
    ...
    /Unsupported STARCODE_SHOWCASE_ANDROID_ABI/u,
  );
});
...
it("maps capture scenes to the real application routes", () => {
  assert.equal(showcaseSceneUrl("threads", "environment-1"), "starcode-dev://");
  assert.equal(
    showcaseSceneUrl("environments", "environment-1"),
    "starcode-dev://settings/environments",
  );
  assert.equal(
    showcaseSceneUrl("thread", "environment-1"),
    "starcode-dev://threads/environment-1/remote-command-center",
  );
  assert.equal(
    showcaseSceneUrl("terminal", "environment-1"),
    "starcode-dev://threads/environment-1/remote-command-center/terminal?terminalId=term-1",
  );
  assert.equal(
    showcaseSceneUrl("review", "environment-1"),
    "starcode-dev://threads/environment-1/remote-command-center/review",
  );
});
