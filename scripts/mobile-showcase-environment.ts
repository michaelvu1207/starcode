// @effect-diagnostics nodeBuiltinImport:off globalDate:off - This host-side fixture creates an isolated local starcode environment.
...
export const SHOWCASE_PROJECT_ID = "starcode";
...
export const SHOWCASE_TERMINAL_BUFFER = [
  "\u001b[38;5;75m~/Code/starcode\u001b[0m \u001b[38;5;212mfeat/remote-command-center\u001b[0m",
  "$ vp test run --changed",
  "",
  "  \u001b[38;5;117mstarcode-mobile\u001b[0m       184 passed",
...
const PROJECT_FAVICONS = {
  starcode: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 128">
...
export const SHOWCASE_PROJECTS = [
  {
    ...
    favicon: PROJECT_FAVICONS.starcode,
  },
  {
    id: "react",
    title: "React",
...
async function seedStarcodeWorkspace(workspaceRoot: string): Promise<void> {
  ...
  await NodeFSP.writeFile(
    ...
    `${JSON.stringify({ name: "starcode", private: true, scripts: { test: "vp test" } }, null, 2)}\n`,
  );
  await NodeFSP.writeFile(NodePath.join(workspaceRoot, "favicon.svg"), PROJECT_FAVICONS.starcode);
  await NodeFSP.writeFile(
...
async function seedCompanionWorkspace(input: {
  ...
  await NodeFSP.writeFile(
    ...
    `# ${input.title}\n\nSeeded by the starcode mobile screenshot harness.\n`,
  );
  await initializeRepository({
...
export async function seedShowcaseEnvironment(input: {
  ...
  if (primaryProject.id === SHOWCASE_PROJECT_ID) {
    await seedStarcodeWorkspace(workspaceRoot);
  }
  await Promise.all(
    projects
