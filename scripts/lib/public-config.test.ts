...
describe("loadRepoEnv", () => {
  it("does not project cloud configuration for an unconfigured clone", () => {
    ...
    expect(env.STARCODE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.STARCODE_CLERK_CLI_OAUTH_CLIENT_ID).toBeUndefined();
    expect(env.VITE_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY).toBeUndefined();
    expect(env.STARCODE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.VITE_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.EXPO_PUBLIC_CLERK_JWT_TEMPLATE).toBeUndefined();
    expect(env.STARCODE_RELAY_URL).toBeUndefined();
    expect(env.VITE_STARCODE_RELAY_URL).toBeUndefined();
    expect(env.STARCODE_MOBILE_OTLP_TRACES_URL).toBeUndefined();
    expect(env.STARCODE_MOBILE_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.STARCODE_MOBILE_OTLP_TRACES_TOKEN).toBeUndefined();
    ...
    expect(env.STARCODE_RELAY_CLIENT_OTLP_TRACES_URL).toBeUndefined();
    expect(env.STARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET).toBeUndefined();
    expect(env.STARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN).toBeUndefined();
  ...
  it("applies process, root local, and root precedence in that order", () => {
    ...
    NodeFS.writeFileSync(
      ...
      "STARCODE_CLERK_PUBLISHABLE_KEY=pk_root\nSTARCODE_CLERK_JWT_TEMPLATE=template_root\nSTARCODE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_root\nSTARCODE_RELAY_URL=https://root.example.test\n",
    );
    NodeFS.writeFileSync(
      NodePath.join(repoRoot, ".env.local"),
      "STARCODE_CLERK_PUBLISHABLE_KEY=pk_local\nSTARCODE_CLERK_JWT_TEMPLATE=template_local\nSTARCODE_CLERK_CLI_OAUTH_CLIENT_ID=oauth_local\nSTARCODE_RELAY_URL=https://local.example.test\n",
    );

    expect(loadRepoEnv({ baseEnv: {}, repoRoot }).STARCODE_RELAY_URL).toBe(
      "https://local.example.test",
    );
    expect(
      loadRepoEnv({
        baseEnv: {
          STARCODE_CLERK_PUBLISHABLE_KEY: "pk_ci",
          STARCODE_CLERK_JWT_TEMPLATE: "template_ci",
          STARCODE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_ci",
          STARCODE_RELAY_URL: "https://ci.example.test",
        },
        repoRoot,
      }),
    ).toMatchObject({
      ...
      STARCODE_RELAY_URL: "https://ci.example.test",
      VITE_STARCODE_RELAY_URL: "https://ci.example.test",
    });
  });
  ...
  it("accepts legacy framework aliases as root overrides", () => {
    expect(
      resolvePublicConfig({
        ...
        STARCODE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_canonical",
        VITE_STARCODE_RELAY_URL: "https://legacy.example.test",
  ...
  it("projects canonical relay client tracing values to web build aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          STARCODE_RELAY_CLIENT_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          STARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET: "relay-client-traces",
          STARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "relay-client-token",
        },
  ...
  it("projects canonical mobile tracing values to Expo public aliases", () => {
    expect(
      loadRepoEnv({
        baseEnv: {
          STARCODE_RELAY_URL: "https://relay.example.test",
          STARCODE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
          STARCODE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
          STARCODE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
        },
        repoRoot: makeTemporaryDirectory(),
      }),
    ).toEqual({
      STARCODE_RELAY_URL: "https://relay.example.test",
      VITE_STARCODE_RELAY_URL: "https://relay.example.test",
      STARCODE_MOBILE_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
      STARCODE_MOBILE_OTLP_TRACES_DATASET: "mobile-traces",
      STARCODE_MOBILE_OTLP_TRACES_TOKEN: "mobile-token",
      EXPO_PUBLIC_OTLP_TRACES_URL: "https://api.axiom.co/v1/traces",
...
function makeTemporaryDirectory() {
  const directory = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "starcode-public-config-"));
