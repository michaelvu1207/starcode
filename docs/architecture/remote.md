...
This document describes the target architecture for first-class remote environments in starcode.
...
- Keep the starcode server as the execution boundary.
...
starcode already has a clean runtime boundary: the client talks to a starcode server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one starcode server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one starcode server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the starcode runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running starcode server instance.
...
How does the client speak WebSocket to a starcode server?
...
- best fit for hosted or self-managed remote starcode deployments
...
For starcode, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a starcode-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote starcode server
...
How does a starcode server come to exist on the target machine?
...
The user or operator already runs starcode on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in starcode:
...
The recommended starcode flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies starcode availability.
3. Desktop launches or reuses a remote starcode server.
...
This is the inverse of remote launch: a local starcode server is already running, and the client publishes it through a tunnel.
...
- A manually hosted starcode server might be reached through direct `wss`.
...
starcode already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
starcode should not copy that part.

starcode already has the right runtime boundary:

- one starcode server per environment
- ordinary HTTP/WebSocket between client and environment

So starcode should borrow Zed's launch discipline, not its transport protocol.
