...
- `node apps/server/scripts/starcode-sqlite-state.ts <query|exec> --base-dir <path> ...` — Inspects or seeds an isolated starcode SQLite database; writes create a private backup first.
...
- Desktop production windows load the bundled UI from `starcode://app/index.html` (not a `127.0.0.1` document URL).
...
- Signed macOS builds also require `STARCODE_APPLE_TEAM_ID` and
  `STARCODE_MACOS_PROVISIONING_PROFILE`. The passkey RP domain is derived from
  `STARCODE_CLERK_PUBLISHABLE_KEY` unless `STARCODE_CLERK_PASSKEY_RP_DOMAINS` overrides it.
...
Set `STARCODE_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `13773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `STARCODE_DEV_INSTANCE`)
- Example: `STARCODE_DEV_INSTANCE=branch-a vp run dev:desktop`

If you want full control instead of hashing, set `STARCODE_PORT_OFFSET` to a numeric offset.
