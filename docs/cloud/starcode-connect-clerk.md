# starcode Connect Clerk Setup

starcode Connect uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `t3-relay` template with the shared
`starcode-relay` audience.

## Application Keys

starcode Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:

```dotenv
STARCODE_CLERK_PUBLISHABLE_KEY=<publishable key>
STARCODE_CLERK_JWT_TEMPLATE=<JWT template name>
STARCODE_CLERK_CLI_OAUTH_CLIENT_ID=<public OAuth application client ID>
STARCODE_RELAY_URL=https://relay.example.com
```
...
should set `STARCODE_CLERK_PUBLISHABLE_KEY`, `STARCODE_CLERK_JWT_TEMPLATE`,
`STARCODE_CLERK_CLI_OAUTH_CLIENT_ID`, and `STARCODE_RELAY_URL` before building. EAS preview and
...
absent, the `starcode connect` CLI command group is omitted. The bundled server still accepts runtime
...
deployment defaults.
`vp run --filter starcode-relay deploy` invokes Alchemy from the relay directory, so Alchemy loads
...
The `starcode connect` commands authorize a headless environment with a separate Clerk OAuth application.
This uses an OAuth public client with PKCE, so the CLI stores no client secret.

In **Clerk Dashboard > OAuth applications**:

1. Create an OAuth application for the starcode CLI.
...
5. Set `STARCODE_CLERK_CLI_OAUTH_CLIENT_ID` in the repository-root `.env` file and release build
...
```sh
starcode connect login
starcode connect link
starcode connect status
starcode connect unlink
starcode connect logout
starcode serve
```

`starcode connect login` opens the Clerk authorization flow and stores the CLI credential without enabling
cloud exposure. `starcode connect link` installs the pinned managed `cloudflared` binary when needed,
authorizes when needed, and records durable intent to expose the environment. It works without a
running starcode server. The next `starcode serve` or `starcode start` reconciles the relay link and launches the
managed tunnel. `starcode connect unlink` records disabled intent immediately, stops a reachable running
connector, and attempts to revoke the relay-side environment record. It retains the stored CLI
authorization so `starcode connect link` can re-enable exposure without another browser flow. `starcode connect
...
logout leaves it running; manage it with `starcode service status`, `install`, `update`, and `uninstall`.

The current OAuth callback listener binds to loopback port `34338`. When running the CLI over SSH,
forward that port before running `starcode connect login` or `starcode connect link`:
...
| ------- | ----------------------------- |
| Name    | `t3-relay`                    |
| Claims  | `{ "aud": "starcode-relay" }` |

Set `STARCODE_CLERK_JWT_TEMPLATE=t3-relay` in the repository-root `.env`, and set
`CLERK_JWT_AUDIENCE=starcode-relay` in `infra/relay/.env`. Define `CLERK_JWT_TEMPLATE` and
`CLERK_JWT_AUDIENCE` in the production relay deployment environment as well. The stable `aud` value
is shared by production and non-production relay stages. The client-facing `STARCODE_RELAY_URL` still
...
Local desktop development uses `starcode-dev://app`, while packaged builds use `starcode://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `starcode-dev://app`; the production Clerk instance should only need
`starcode://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
curl -X PATCH https://api.clerk.com/v1/instance \
  ...
  -d '{"allowed_origins":["starcode://app"]}'
```
...
The production macOS bundle ID is `com.starcode.starcode`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.starcode.starcode` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.starcode.starcode`.
...
```dotenv
STARCODE_APPLE_TEAM_ID=ABC1234567
STARCODE_MACOS_PROVISIONING_PROFILE=/absolute/path/to/starcode.provisionprofile
# Optional: comma-separated override when Clerk's RP ID differs from the Frontend API hostname.
STARCODE_CLERK_PASSKEY_RP_DOMAINS=example.clerk.accounts.dev,clerk.example.com
```

When `STARCODE_CLERK_PASSKEY_RP_DOMAINS` is absent, the build derives the RP domain from
`STARCODE_CLERK_PUBLISHABLE_KEY`. Signed macOS builds fail early if the Team ID, provisioning profile,
...
installed app executable with `VITE_DEV_SERVER_URL` and `STARCODE_PORT` set. Rebuild the signed app
...
STARCODE_PORT=13773 \
  "/Applications/starcode (Alpha).app/Contents/MacOS/starcode (Alpha)"
```
...
```sh
codesign --verify --deep --strict "/Applications/starcode (Alpha).app"
codesign -d --entitlements :- "/Applications/starcode (Alpha).app"
```
...
Approved signed-in users manage starcode Connect under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the starcode Connect controls on the Connections page.

On mobile, signed-out users open **Settings > starcode Account** to reach `/settings/waitlist` within the
