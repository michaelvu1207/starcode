...
This guide is for people who want to use more than one Claude setup in starcode.
...
In starcode Settings, your Claude provider can stay like this:
...
An empty `Claude HOME path` means starcode uses your normal home directory.
...
Then add another Claude provider in starcode:
...
starcode only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so starcode keeps separate Claude homes isolated instead of
...
Add or edit a Claude provider in starcode Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. starcode stores the value as a server secret and does not
...
starcode does not need a special Claude Code Router provider. Treat the router as a Claude
environment.
...
2. Add or configure a Claude provider in starcode.
