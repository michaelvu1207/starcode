...
This guide is for people who want to use more than one Codex account in starcode.
...
- both accounts can see the same starcode/Codex sessions
...
In starcode Settings, name it something obvious:
...
In starcode Settings, add another Codex provider:
...
starcode shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, starcode treats it
