...
- `starcode-relay-traces-prod`, an OpenTelemetry trace dataset for Worker requests
- `starcode-relay-otel-ingest-prod`, a dataset-scoped ingest token bound to the Worker
- `starcode-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`starcode-relay-traces-dev-julius` for a personal stage.
