...
The screenshot harness runs the real mobile application against three disposable local starcode
...
the software keyboard, and supplies deterministic starcode Connect discovery rows to the real
...
1. Creates three temporary starcode base directories and starts a local server for each on an available
   port.
2. Creates starcode, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic starcode review diff.
...
ARM64 by default for local Apple Silicon development; CI sets `STARCODE_SHOWCASE_ANDROID_ABI=x86_64` so the
...
Run Metro separately:
    ...
    pnpm --filter @starcode/mobile showcase
...
The Pending rows use the production offline outbox and point at the real starcode and React fixture
