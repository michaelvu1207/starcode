...
const configuredRelayUrl = repoEnv.VITE_STARCODE_RELAY_URL?.trim() || "";
...
const sourcemapEnv = process.env.STARCODE_WEB_SOURCEMAP?.trim().toLowerCase();
...
// as hot patches. Opt-in while experimental: STARCODE_BUNDLED_DEV=1 pnpm dev:web
const bundledDevEnv = process.env.STARCODE_BUNDLED_DEV?.trim().toLowerCase();
...
export default defineConfig(() => {
  return {
    ...
    define: {
      ...
      "import.meta.env.VITE_STARCODE_RELAY_URL": JSON.stringify(configuredRelayUrl),
