...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with starcode-specific exact filename icons", () => {
    assert.equal(
      resolvePierreIconForEntry("package.json", "file")?.name,
      "starcode-file-icon-package-json",
    );
    assert.equal(
      resolvePierreIconForEntry("config/tsconfig.json", "file")?.name,
      "starcode-file-icon-tsconfig",
    );
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "starcode-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("CLAUDE.md", "file")?.name, "starcode-file-icon-claude");
    assert.equal(resolvePierreIconForEntry("README.md", "file")?.name, "starcode-file-icon-readme");
    assert.equal(
      resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name,
      "starcode-file-icon-pnpm",
    );
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(Object.values(STARCODE_PIERRE_ICONS.byFileName));
    for (const iconName of customIconNames) {
      assert.include(STARCODE_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
