...
import { MessageSimplifyButton } from "./MessageSimplifyButton";
import { MessageSimplifyPromptDialog } from "./MessageSimplifyPromptDialog";
import {
  isAtomCommandInterrupted,
  squashAtomCommandFailure,
} from "@starcode/client-runtime/state/runtime";
import { useAtomCommand } from "../../state/use-atom-command";
import { messageSimplificationEnvironment } from "../../state/messageSimplification";
...
import { ThreadAssistantMessageLayout, ThreadUserMessageLayout } from "./ThreadMessageLayout";
...
interface AssistantSimplificationState {
  readonly pending: boolean;
  readonly showingSummary: boolean;
  readonly summary: string | null;
}

interface TimelineSimplificationState {
  readonly byMessageId: ReadonlyMap<MessageId, AssistantSimplificationState>;
  readonly onToggle: (messageId: MessageId, anchor: HTMLButtonElement) => void;
  readonly onEditPrompt: () => void;
}

const TimelineRowCtx = createContext<TimelineRowSharedState>(null!);
const TimelineRowActivityCtx = createContext<TimelineRowActivityState>(null!);
const TimelineSimplificationCtx = createContext<TimelineSimplificationState>(null!);
const TIMELINE_LIST_HEADER = <div className="h-3 sm:h-4" />;
const TIMELINE_LIST_FOOTER = <div className="h-3 sm:h-4" />;
const EMPTY_TIMELINE_SKILLS: ReadonlyArray<Pick<ServerProviderSkill, "name" | "displayName">> = [];
const MESSAGE_SIMPLIFICATION_INSTRUCTIONS_STORAGE_KEY =
  "starcode:message-simplification-instructions:v1";
const MessageSimplificationInstructionsStorage = Schema.String.check(
  Schema.isNonEmpty(),
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const simplifyMessage = useAtomCommand(messageSimplificationEnvironment.simplify, {
    reportFailure: false,
  });
  const [simplificationInstructions, setSimplificationInstructions] = useLocalStorage(
    MESSAGE_SIMPLIFICATION_INSTRUCTIONS_STORAGE_KEY,
    DEFAULT_MESSAGE_SIMPLIFICATION_INSTRUCTIONS,
    MessageSimplificationInstructionsStorage,
  );
  const [simplificationPromptOpen, setSimplificationPromptOpen] = useState(false);
  const simplificationPromptRevisionRef = useRef(0);
  const [assistantSimplifications, setAssistantSimplifications] = useState<
    ReadonlyMap<MessageId, AssistantSimplificationState>
  >(() => new Map());
  const assistantSimplificationsRef = useRef(assistantSimplifications);
  assistantSimplificationsRef.current = assistantSimplifications;

  const updateAssistantSimplifications = useCallback(
    (
      update: (
        current: ReadonlyMap<MessageId, AssistantSimplificationState>,
      ) => ReadonlyMap<MessageId, AssistantSimplificationState>,
    ) => {
      const next = update(assistantSimplificationsRef.current);
      assistantSimplificationsRef.current = next;
      setAssistantSimplifications(next);
    },
    [],
  );

  const onToggleAssistantSimplification = useCallback(
    (messageId: MessageId, anchor: HTMLButtonElement) => {
      const current = assistantSimplificationsRef.current.get(messageId);
      if (current?.pending) return;
      if (current?.summary) {
        updateAssistantSimplifications((all) => {
          const next = new Map(all);
          next.set(messageId, { ...current, showingSummary: !current.showingSummary });
          return next;
        });
        return;
      }
      ...
      updateAssistantSimplifications((all) => {
        const next = new Map(all);
        next.set(messageId, { pending: true, showingSummary: false, summary: null });
        return next;
      });

      const promptRevision = simplificationPromptRevisionRef.current;

      void simplifyMessage({
        ...
        if (promptRevision !== simplificationPromptRevisionRef.current) return;
        ...
        anchoredToastManager.add({
          ...
          timeout: 3_000,
          title: "Could not simplify",
          description:
            error instanceof Error
              ? error.message
              : "The assistant response could not be simplified.",
        });
      });
    },
    [
      ...
      simplifyMessage,
      threadRef,
      updateAssistantSimplifications,
    ],
  );

  const onSaveSimplificationInstructions = useCallback(
    (instructions: string) => {
      simplificationPromptRevisionRef.current += 1;
      setSimplificationInstructions(instructions);
      updateAssistantSimplifications(() => new Map());
    },
    [setSimplificationInstructions, updateAssistantSimplifications],
  );
  ...
  const simplificationState = useMemo<TimelineSimplificationState>(
    () => ({
      byMessageId: assistantSimplifications,
      onToggle: onToggleAssistantSimplification,
      onEditPrompt: () => setSimplificationPromptOpen(true),
    }),
    [assistantSimplifications, onToggleAssistantSimplification],
  );
  ...
  return (
    <TimelineRowCtx value={sharedState}>
      <TimelineRowActivityCtx value={activityState}>
        <TimelineSimplificationCtx value={simplificationState}>
          ...
          <MessageSimplifyPromptDialog
            open={simplificationPromptOpen}
            instructions={simplificationInstructions}
            onOpenChange={setSimplificationPromptOpen}
            onSave={onSaveSimplificationInstructions}
          />
        </TimelineSimplificationCtx>
...
function AssistantTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "message" }> }) {
  const ctx = use(TimelineRowCtx);
  const simplification = use(TimelineSimplificationCtx);
  const simplified = simplification.byMessageId.get(row.message.id);
  const messageText = row.message.text || (row.message.streaming ? "" : "(empty response)");
  const displayedText = simplified?.showingSummary
    ? (simplified.summary ?? messageText)
    : messageText;
    ...
    <ThreadAssistantMessageLayout>
      ...
      {row.showAssistantMeta ? (
        <div className="mt-1.5 flex items-center gap-2 text-xs tabular-nums opacity-0 transition-opacity duration-200 focus-within:opacity-100 group-hover/assistant:opacity-100">
          {row.showAssistantCopyButton && row.message.text?.trim() ? (
            <MessageSimplifyButton
              pending={simplified?.pending ?? false}
              showingSummary={simplified?.showingSummary ?? false}
              onToggle={(anchor) => simplification.onToggle(row.message.id, anchor)}
              onEditPrompt={simplification.onEditPrompt}
            />
          ) : null}
          <AssistantCopyButton row={row} text={displayedText} />
...
function workEntryIconName(workEntry: TimelineWorkEntry): WorkEntryIconName {
  ...
  if (workEntry.itemType === "file_read" || workEntry.itemType === "image_view") return "eye";
...
function activityPhraseForWorkEntry(
  ...
  const kind = activityKindFromItemType({
    ...
    isApproval:
      workEntry.sourceActivityKind === "approval.requested" ||
      workEntry.sourceActivityKind === "approval.resolved",
  ...
  if (kind === "approval" && phase === "settled") {
    const verb =
