 ...
 * that write no CLI store — cursor and grok — contribute no spend here.
...
export function AccountsUsagePanel() {
  ...
  const selectedCliProvider: CliUsageProvider | null = null;
  const chartGroups = useMemo(
    () =>
      selectedCliProvider === null
        ? view.groups
        : view.groups.map((group) => ({
            ...group,
            cliHistory: {
              ...group.cliHistory,
              providers: group.cliHistory.providers.filter(
                (provider) => provider.provider === selectedCliProvider,
              ),
            },
          })),
    [selectedCliProvider, view.groups],
  );
  const chart = useMemo(() => buildUsageDailyChartView(chartGroups, range), [chartGroups, range]);
  ...
  const providers = [...history.providers]
    .filter((provider) => selectedCliProvider === null || provider.provider === selectedCliProvider)
    .sort(
      (left, right) =>
  ...
  const displayedWindow =
    selectedCliProvider === null ? fleetWindow : (providers[0]?.windows.last30Days ?? fleetWindow);
  ...
  const gridProps = {
    providers,
    totalCostUsd: displayedWindow.costUsd,
    pending: history.pending,
  };

  return (
    <SettingsPageContainer className="max-w-none gap-5">
      <SettingsSection
        title="Usage"
        headerAction={
          history.scanning ? (
            ...
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
              {history.pending || !history.reported ? (
                ...
                <p className="text-4xl font-semibold tabular-nums tracking-tight text-foreground">
                  {formatUsd(displayedWindow.costUsd)}
                </p>
              )}
              {history.costIsFloor ? <FloorBadge totals={displayedWindow} /> : null}
            </div>
      ...
      <p className="flex flex-wrap items-center gap-x-1.5 px-3 text-[11px] leading-[1.5] text-muted-foreground/60 sm:px-4">
        ...
        <span>
          ...
          session files · spend covers Claude Code and Codex; subscription allowances appear with
          each account.
