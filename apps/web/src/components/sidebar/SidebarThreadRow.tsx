...
export function SidebarThreadRow({
  ...
  readonly archiveMode?: "archive" | "unarchive";
}): ReactNode {
      ...
      <Tooltip>
        ...
        >
          ...
          <span className="relative flex h-6 min-w-7 shrink-0 items-center justify-end">
            ...
            >
              {/* The archive transition earns its own button rather than staying
                    one level down in the menu: active threads leave in one click,
                    and archived threads can be restored from the same place. */}
              <button
                type="button"
                aria-label={archiveMode === "unarchive" ? "Unarchive thread" : "Archive thread"}
                title={archiveMode === "unarchive" ? "Unarchive thread" : "Archive thread"}
