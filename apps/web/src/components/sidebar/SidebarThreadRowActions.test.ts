...
describe("canForkConversation", () => {
  it("carries the conversation for a Pi thread that has spoken", () => {
    expect(canForkConversation({ driverKind: "pi", thread: shell({}) })).toBe(true);
  });
  ...
  it("still says setup-only for a Pi thread with nothing behind it", () => {
    expect(
      canForkConversation({
        driverKind: "pi",
        thread: shell(null),
  ...
  it("never carries a conversation on a driver with no fork primitive", () => {
    ...
    // turn down.
    for (const driverKind of ["claudeAgent", "codex", "opencode", "gemini", "claude", null]) {
      expect(
        canForkConversation({
          driverKind,
          thread: shell({}),
