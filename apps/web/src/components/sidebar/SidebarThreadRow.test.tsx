...
describe("SidebarThreadRow", () => {
  ...
  it("turns the direct action into restore while browsing archived threads", () => {
    const markup = render({ archiveMode: "unarchive" });

    expect(markup).toContain('data-testid="sidebar-v2-row-archive"');
    expect(markup).toContain('aria-label="Unarchive thread"');
    expect(markup).not.toContain('aria-label="Archive thread"');
  });
