...
/**
 * Drives the primary environment's starcode Connect link. starcode Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update starcode Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[starcode-connect] Could not update starcode Connect", {
      message,
      traceId,
      cause,
    });
    ...
    toastManager.add({
      type: "error",
      title: "Could not update starcode Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn starcode Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to starcode Connect before enabling this."));
        return false;
      }
