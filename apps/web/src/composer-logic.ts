...
/** Parse `/goal <objective>` while keeping ordinary slash-prefixed prose intact. */
export function parseComposerGoalCommand(text: string): string | null {
  const match = /^\/goal\s+([\s\S]+)$/i.exec(text.trim());
  const objective = match?.[1]?.trim();
  return objective && objective.length <= 4_000 ? objective : null;
}
