...
const HOSTED_WEB_CHANNEL_COOKIE = "starcode_web_channel";
...
export const config: VercelConfig = {
  buildCommand:
    'vp run --filter @starcode/web build && node ../../scripts/apply-web-brand-assets.ts --channel "${VITE_HOSTED_APP_CHANNEL:-latest}"',
  git: {
    deploymentEnabled: false,
  },
  installCommand:
    "npm install -g vite-plus && vp install --filter '@starcode/scripts...' --filter '@starcode/web...'",
  routes: [
    {
      src: "/__starcode/channel",
