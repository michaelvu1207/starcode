...
public final class StarcodeKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("StarcodeKeyboardCommands")

    View(StarcodeKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: StarcodeKeyboardCommandsView, commands: [String]) in
...
public final class StarcodeKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.starcodeFirstResponder == nil else { return }
...
private extension UIView {
  var starcodeFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.starcodeFirstResponder { return responder }
    }
    return nil
  }
}
