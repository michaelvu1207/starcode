...
#import "StarcodeMarkdownTextShadowNode.h"
#import "StarcodeMarkdownTextComponentDescriptor.h"
...
static void StarcodeMarkdownTextApplyParagraphStyles(
    NSMutableAttributedString *attributedString,
    const std::vector<StarcodeMarkdownTextParagraphStyleRange> &styleRanges)
{
...
static void StarcodeMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
    const std::vector<StarcodeMarkdownTextAttachmentRange> &attachmentRanges,
    NSDictionary<NSString *, UIImage *> *images)
{
  for (const auto &attachmentRange : attachmentRanges) {
    ...
    const CGFloat attachmentSize = StarcodeMarkdownTextAttachmentSize(attachmentRange);
    attachment.bounds = CGRectMake(
        0,
        StarcodeMarkdownTextAttachmentBaselineOffset(attachmentRange),
        attachmentSize,
        attachmentSize);
...
@protocol StarcodeMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface StarcodeMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<StarcodeMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<StarcodeMarkdownOutsideTapTarget>)target;

@end

static const void *StarcodeMarkdownOutsideTapCoordinatorKey =
    &StarcodeMarkdownOutsideTapCoordinatorKey;

@implementation StarcodeMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<StarcodeMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<StarcodeMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, StarcodeMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<StarcodeMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static StarcodeMarkdownOutsideTapCoordinator *
StarcodeMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  StarcodeMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, StarcodeMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[StarcodeMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        StarcodeMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface StarcodeMarkdownText () <RCTStarcodeMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface StarcodeMarkdownText () <StarcodeMarkdownOutsideTapTarget>
@end

@implementation StarcodeMarkdownText {
  UIView * _view;
  UITextView * _textView;
  StarcodeMarkdownTextShadowNode::ConcreteState::Shared _state;
...
{
  return concreteComponentDescriptorProvider<StarcodeMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const StarcodeMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    StarcodeMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, StarcodeMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [StarcodeMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<StarcodeMarkdownTextProps const>(_props);
  ...
  StarcodeMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  StarcodeMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::StarcodeMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::StarcodeMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<StarcodeMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
{
  const auto &oldViewProps = *std::static_pointer_cast<StarcodeMarkdownTextProps const>(_props);
  const auto &newViewProps = *std::static_pointer_cast<StarcodeMarkdownTextProps const>(props);
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == StarcodeMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == StarcodeMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == StarcodeMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == StarcodeMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const StarcodeMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (StarcodeMarkdownTextRun*)getTouchChild:(CGPoint)location
{
  ...
  for (UIView* child in self.subviews) {
    if (![child isKindOfClass:[StarcodeMarkdownTextRun class]]) {
      continue;
    }

    StarcodeMarkdownTextRun* textChild = (StarcodeMarkdownTextRun*)child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::StarcodeMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::StarcodeMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> StarcodeMarkdownTextCls(void)
{
  return StarcodeMarkdownText.class;
}
