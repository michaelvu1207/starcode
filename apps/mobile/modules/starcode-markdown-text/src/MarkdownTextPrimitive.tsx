...
import StarcodeMarkdownTextRunNativeComponent from "./StarcodeMarkdownTextRunNativeComponent";
import StarcodeMarkdownTextNativeComponent from "./StarcodeMarkdownTextNativeComponent";
...
function MarkdownTextPrimitiveChild({ style, children, ...rest }: MarkdownTextPrimitiveProps) {
  ...
  const nativeChildren = React.Children.toArray(children).map((child) => {
    ...
    return (
      ...
      <StarcodeMarkdownTextRunNativeComponent
  ...
  if (!isAncestor) {
    return (
      <TextAncestorContext.Provider value={contextValue}>
        <StarcodeMarkdownTextNativeComponent
          {...textDefaults}
          {...rest}
