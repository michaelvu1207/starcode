...
export function NativeMarkdownSelectableText(props: {
  ...
  // StarcodeMarkdownText only rebuilds its attributed string during native layout. A
      ...
      {keyedRuns.map(({ key, run, text }) => {
        ...
        return (
          <MarkdownTextPrimitive
            ...
            nativeID={
              run.fileIcon
                ? `starcode-file:${Image.resolveAssetSource(markdownFileIconSource(run.fileIcon)).uri}`
                : run.skillName
                  ? "starcode-skill:sf:cube"
                  : undefined
            }
