import { ApprovalRequestId, isToolLifecycleItemType } from "@starcode/contracts";
import type {
...
import { formatDuration } from "@starcode/shared/orchestrationTiming";
import {
  ...
  for (const entry of entries) {
    ...
    for (const key of keys) {
      ...
      if (
        ...
        merged = true;
        if (isTerminalLifecycleActivityKind(next.activityKind)) {
          closeEntry(next);
        }
        break;
      }
    ...
    collapsed.push(entry);
    if (isTerminalLifecycleActivityKind(entry.activityKind)) {
      continue;
    }
    for (const key of keys) {
...
function isTerminalLifecycleActivityKind(kind: OrchestrationThreadActivity["kind"]): boolean {
  return kind === "tool.completed" || kind === "approval.resolved";
}
...
function shouldCollapseToolLifecycleEntries(
  ...
  if (isTerminalLifecycleActivityKind(previous.activityKind)) {
    return false;
  }
  if (
...
function workEntryHeading(workEntry: DerivedWorkLogEntry): string {
  const kind = activityKindFromItemType({
    ...
    isApproval:
      workEntry.activityKind === "approval.requested" ||
      workEntry.activityKind === "approval.resolved",
