...
/**
 * Installs StarCode as a per-user background service: systemd + linger on
 * Linux, or a LaunchAgent on macOS. The service runs a stable or pinned
 * runtime — never an ephemeral package-manager cache whose eviction could
 * break startup.
 */
...
export const BOOT_SERVICE_LAUNCH_AGENT_LABEL = "com.starcode.server";
...
export function renderBootServiceLaunchAgent(plan: BootServicePlan): string {
  ...
  return [
    ...
    `    <string>${value(plan.starcodeEntryPath)}</string>`,
    "    <string>serve</string>",
    "  </array>",
    "  <key>EnvironmentVariables</key>",
    "  <dict>",
    "    <key>STARCODE_HOME</key>",
...
function isCurrentLaunchAgentDefinition(plist: string, plan: BootServicePlan): boolean {
  ...
  const planWithoutPath: BootServicePlan = {
    ...
    starcodeEntryPath: plan.starcodeEntryPath,
...
export class BootServiceCommandError extends Schema.TaggedErrorClass<BootServiceCommandError>()(
  ...
  override get message(): string {
    ...
    return this.supervisor === "launchd"
      ? `${detail} If macOS blocked the background item, allow StarCode in System Settings > General > Login Items & Extensions.`
      : detail;
  }
}
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the starcode background service.";
  }
}
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
   ...
   * Stable StarCode checkouts and bundles are used as-is. Ephemeral package
   * manager entries are replaced by a managed runtime so a service never
   * depends on a cache that can disappear.
   */
  ...
  const useManagedRuntime = isEphemeralCacheEntry(host.cliEntryPath);
  const plannedRuntimeEntryPath = useManagedRuntime ? runtimePaths.entryPath : host.cliEntryPath;
  const plannedEntryPath =
    platform === "darwin" && launchAgent !== null && useManagedRuntime
  ...
  const install: BootService["Service"]["install"] = Effect.gen(function* () {
    ...
    yield* fs.makeDirectory(unitDir, { recursive: true }).pipe(
      Effect.andThen(
        platform === "darwin"
          ? (useManagedRuntime
              ? writeActiveRuntimeLink(runtimePaths.versionDir)
              : Effect.void
            ).pipe(Effect.andThen(writeUnit(renderBootServiceLaunchAgent(plan))))
  ...
  const status: BootService["Service"]["status"] = Effect.gen(function* () {
    ...
    if (platform === "darwin" && launchAgent !== null) {
      const activeRuntimeIsCurrent = useManagedRuntime
        ? yield* fs.readLink(launchAgent.activeRuntimeLinkPath).pipe(
