...
it.effect("prefers a runtime relay URL override over the statically injected value", () =>
  Effect.gen(function* () {
    const relayUrl = yield* makeRelayUrlConfig("https://embedded.example.test").pipe(
      provideEnv({ STARCODE_RELAY_URL: "https://runtime.example.test///" }),
    );
...
it.effect("rejects an insecure runtime relay URL override", () =>
  makeRelayUrlConfig("https://embedded.example.test").pipe(
    provideEnv({ STARCODE_RELAY_URL: "http://runtime.example.test" }),
    Effect.flip,
  ),
);
  ...
  Effect.gen(function* () {
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ STARCODE_HOSTED_APP_URL: "https://nightly.app.t3.codes" }),
      ),
      "https://nightly.app.t3.codes",
    );
    assert.equal(
      yield* hostedAppUrlConfig.pipe(
        provideEnv({ STARCODE_HOSTED_APP_URL: "http://localhost:5733" }),
      ),
      "http://localhost:5733",
    );
  }),
  ...
  Effect.gen(function* () {
    for (const value of [
      ...
      const result = yield* hostedAppUrlConfig.pipe(
        provideEnv({ STARCODE_HOSTED_APP_URL: value }),
        Effect.result,
      );
...
it.effect("prefers runtime Clerk OAuth config overrides over statically injected values", () =>
  Effect.gen(function* () {
    const config = yield* makeCloudCliOAuthConfig({
      ...
      provideEnv({
        STARCODE_CLERK_PUBLISHABLE_KEY: "pk_test_cnVudGltZS5leGFtcGxlLnRlc3Qk",
        STARCODE_CLERK_CLI_OAUTH_CLIENT_ID: "oauth_client_runtime",
      }),
    );
...
it("resolves relay client tracing from runtime config with build-time fallback", () => {
  ...
  assert.deepEqual(
    resolveRelayClientTracingConfig(
      {
        STARCODE_RELAY_CLIENT_OTLP_TRACES_URL: "https://runtime.example.test/v1/traces",
        STARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        STARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
    ),
    {
  ...
  assert.equal(
    resolveRelayClientTracingConfig(
      {
        STARCODE_RELAY_CLIENT_OTLP_TRACES_URL: "http://insecure.example.test/v1/traces",
        STARCODE_RELAY_CLIENT_OTLP_TRACES_DATASET: "runtime-dataset",
        STARCODE_RELAY_CLIENT_OTLP_TRACES_TOKEN: "runtime-token",
      },
      fallback,
