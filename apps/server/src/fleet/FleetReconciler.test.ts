...
import {
  deriveFleetNodeName,
  collectAvailableFleetBootstrapNodes,
...
describe("fleet client bootstrap", () => {
  it.effect("returns available remote nodes without waiting for an offline member", () =>
    Effect.gen(function* () {
      const available = {
        nodeId: FleetNodeName.make("available"),
        environmentId: EnvironmentId.make("available"),
        label: "Available",
        endpoint: {
      ...
      const fiber = yield* collectAvailableFleetBootstrapNodes(
        [Effect.succeed(Option.some(available)), Effect.never],
        "2 seconds",
      ).pipe(Effect.forkChild);

      yield* TestClock.adjust("2 seconds");

      assert.deepEqual(yield* Fiber.join(fiber), [available]);
    }),
  );
});
