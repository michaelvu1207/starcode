...
import {
  ...
  type FleetPiAccountImportInput,
...
/** Deliver subscription accounts over an authenticated fleet connection. */
export const importFleetPiAccounts = Effect.fn("FleetClient.importPiAccounts")(function* (input: {
  readonly baseUrl: string;
  readonly credential: string;
  readonly payload: FleetPiAccountImportInput;
}) {
  const client = yield* makeClient(input.baseUrl);
  return yield* client.fleet
    .importAccounts({ headers: bearer(input.credential), payload: input.payload })
    .pipe(Effect.timeout(FLEET_REQUEST_TIMEOUT));
});
