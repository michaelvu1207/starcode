...
const dependencies = [McpInvocationContext.McpInvocationContext, FeatureMapRegistry];
...
export const FeatureMapListTool = readTool(
  Tool.make("feature_map_list", {
    description:
      "List the features on the workbench sky: name, description, the stage each has reached (in-progress, in-dev, in-staging, in-production), the thread doing the work, and which features wait on which. Planned features — ones that exist only as intent — are included and marked. Use this before creating a feature, so you enrich the existing entry instead of adding a duplicate.",
...
export const FeatureCreateTool = writeTool(
  Tool.make("feature_create", {
    description:
      "Add a feature to the sky. Pass threadId to bind it to work already running, which is what makes the star clickable; omit it for work not started yet. Pass planned=true for a feature that is intended rather than under way — it renders as a ghost until a thread is bound to it. Stage defaults to in-progress.",
...
export const FeatureUpdateTool = writeTool(
  Tool.make("feature_update", {
    description:
      "Rename a feature, change its description, or bind it to a thread. Binding a thread turns a planned feature into real work automatically — pass planned explicitly only if you mean to override that.",
