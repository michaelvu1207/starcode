...
const makeDesktopBootstrap = (
  ...
  noBrowser: true,
  port: 4888,
  starcodeHome: "/tmp/starcode-bootstrap-home",
  host: "127.0.0.1",
...
it.layer(NodeServices.layer)("cli config resolution", (it) => {
  ...
  it.effect("falls back to effect/config values when flags are omitted", () =>
    Effect.gen(function* () {
      ...
      const baseDir = join(NodeOS.tmpdir(), "starcode-cli-config-env-base");
          ...
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  STARCODE_LOG_LEVEL: "Warn",
                  STARCODE_MODE: "desktop",
                  STARCODE_PORT: "4001",
                  STARCODE_HOST: "0.0.0.0",
                  STARCODE_HOME: baseDir,
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  STARCODE_NO_BROWSER: "true",
                  STARCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  STARCODE_LOG_WS_EVENTS: "true",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("uses CLI flags when provided", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "starcode-cli-config-flags-base");
      const derivedPaths = yield* deriveExplicitServerPaths(
        baseDir,
            ...
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  ...
                  STARCODE_PORT: "4001",
                  STARCODE_HOST: "0.0.0.0",
                  STARCODE_HOME: join(NodeOS.tmpdir(), "ignored-base"),
                  VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
                  STARCODE_NO_BROWSER: "false",
                  STARCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
                  STARCODE_LOG_WS_EVENTS: "false",
                },
              }),
            ),
            NetService.layer,
  ...
  it.effect("preserves explicit false CLI boolean flags over env and bootstrap values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "starcode-cli-config-false-flags");
        ...
        Effect.provide(
          Layer.mergeAll(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: {
                  STARCODE_BOOTSTRAP_FD: String(fd),
                  STARCODE_NO_BROWSER: "true",
                  STARCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
                  STARCODE_LOG_WS_EVENTS: "true",
                },
              }),
            ),
  ...
  it.effect("uses bootstrap envelope values as fallbacks when flags and env are absent", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = "/tmp/starcode-bootstrap-home";
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
          starcodeHome: baseDir,
          noBrowser: true,
  ...
  it.effect("creates derived runtime directories during config resolution", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "starcode-cli-config-dirs-" });
  ...
  it.effect("applies flag then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "starcode-cli-config-env-wins");
      const fd = yield* openBootstrapFd(
        makeDesktopBootstrap({
          port: 4888,
          host: "127.0.0.2",
  ...
  it.effect("falls back to persisted observability settings when env vars are absent", () =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({
        prefix: "starcode-cli-config-settings-",
      });
  ...
  it.effect("forces noBrowser and disables auto-bootstrap for headless startup presentation", () =>
    Effect.gen(function* () {
      const { join } = yield* Path.Path;
      const baseDir = join(NodeOS.tmpdir(), "starcode-cli-config-headless-base");
      const derivedPaths = yield* deriveExplicitServerPaths(baseDir, undefined);
