...
> {
  switch (command.type) {
    ...
    case "thread.agent.turn.start": {
      const thread = yield* requireThreadNotArchived({
        readModel,
        command,
        threadId: command.threadId,
      });
      const agentRun = thread.agentRuns.find((run) => run.agentRunId === command.agentRunId);
      if (!agentRun) {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `AgentRun '${command.agentRunId}' does not belong to thread '${command.threadId}'.`,
        });
      }
      if (agentRun.provider !== "pi" || agentRun.taskType !== "attached_agent") {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `AgentRun '${command.agentRunId}' is not an interactive Pi AgentRun.`,
        });
      }
      return {
        ...(yield* withEventBase({
    ...
    case "thread.agent.turn.interrupt": {
      const thread = yield* requireThread({
        readModel,
        command,
        threadId: command.threadId,
      ...
      const agentRun = thread.agentRuns.find((run) => run.agentRunId === command.agentRunId);
      if (!agentRun || agentRun.provider !== "pi" || agentRun.taskType !== "attached_agent") {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: `Interactive Pi AgentRun '${command.agentRunId}' does not belong to thread '${command.threadId}'.`,
        });
      }
      return {
        ...(yield* withEventBase({
        ...
        type: "thread.agent-turn-interrupt-requested",
        payload: {
          threadId: command.threadId,
          agentRunId: command.agentRunId,
          createdAt: command.createdAt,
