...
describe("projectAgentRunActivity", () => {
  ...
  it("preserves exact Pi launch options across lifecycle updates", () => {
    const started = projectAgentRunActivity({
      ...
      payload: {
        ...
        options: [{ id: "effort", value: "off" }],
      },
      existing: null,
    });
    assert.deepEqual(started?.options, [{ id: "effort", value: "off" }]);

    const paused = projectAgentRunActivity({
      parentThreadId,
      kind: "task.updated",
      createdAt: "2026-07-30T20:01:00.000Z",
      payload: { taskId: "agent:pi", status: "paused" },
      existing: started,
    });
    assert.deepEqual(paused?.options, [{ id: "effort", value: "off" }]);
  });
  ...
  it("preserves the stable spawn description across progress updates", () => {
    const started = projectAgentRunActivity({
      parentThreadId,
      kind: "task.started",
      createdAt: startedAt,
      payload: {
        taskId: "agent:pi-child",
        taskType: "pi_agent",
        providerDriver: "pi",
        description: "Alpha evidence worker",
      },
      existing: null,
    });
    assert.isNotNull(started);

    const progressed = projectAgentRunActivity({
      parentThreadId,
      kind: "task.progress",
      createdAt: "2026-07-30T20:00:10.000Z",
      payload: {
        taskId: "agent:pi-child",
    ...
    assert.equal(progressed?.description, "Alpha evidence worker");
  });
});
