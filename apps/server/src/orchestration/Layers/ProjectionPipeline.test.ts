...
const BaseTestLayer = makeProjectionPipelinePrefixedTestLayer("starcode-projection-pipeline-test-");

it.layer(BaseTestLayer)("OrchestrationProjectionPipeline", (it) => {
  it.effect("persists the event sequence for projected thread activities", () =>
    Effect.gen(function* () {
      ...
      const rows = yield* sql<{ readonly sequence: number | null }>`
        ...
        WHERE activity_id = 'activity-sequence'
      `;
      assert.deepEqual(rows, [{ sequence: saved.sequence }]);
    }),
  );
  ...
  it.effect("bootstraps all projection states and writes projection rows", () =>
    Effect.gen(function* () {
      ...
      const sequenceRows = yield* sql<{ readonly latestSequence: number }>`
        SELECT MAX(sequence) AS "latestSequence"
        FROM orchestration_events
      `;
      const latestSequence = sequenceRows[0]?.latestSequence;
      if (latestSequence === undefined)
        throw new Error("Expected at least one orchestration event");
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("starcode-base-")))(
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("starcode-projection-attachments-safe-")),
...
it.layer(
  Layer.fresh(
    makeProjectionPipelinePrefixedTestLayer("starcode-projection-attachments-overwrite-"),
  ),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("starcode-projection-attachments-rollback-")),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("starcode-projection-attachments-revert-")),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("starcode-projection-attachments-delete-")),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(makeProjectionPipelinePrefixedTestLayer("starcode-pending-turn-terminal-test-"))(
    ...
    Effect.provide(
      Layer.provideMerge(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "starcode-projection-pipeline-restart-",
        }),
        NodeServices.layer,
      ),
    ),
...
const engineLayer = it.layer(
  OrchestrationEngineLive.pipe(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "starcode-projection-pipeline-engine-dispatch-",
      }),
    ),
    Layer.provideMerge(NodeServices.layer),
  ),
