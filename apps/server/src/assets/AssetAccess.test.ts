...
import { PROJECT_FAVICON_FALLBACK_MARKER } from "@starcode/shared/projectFavicon";
...
const configLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "starcode-asset-access-test-",
});
...
describe("AssetAccess", () => {
  it.effect("issues workspace URLs that resolve the entry file and sibling assets", () =>
    Effect.gen(function* () {
      ...
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "starcode-asset-workspace-",
      });
  ...
  it.effect("rejects workspace files outside the authorized root", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "starcode-asset-root-",
      });
      const outside = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "starcode-asset-outside-",
      });
  ...
  it.effect("preserves non-missing canonical path failures when issuing asset URLs", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "starcode-asset-permission-root-",
      });
      const htmlPath = path.join(root, "report.html");
  ...
  it.effect("issues exact workspace URLs for image previews", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "starcode-asset-image-workspace-",
      });
  ...
  it.effect("preserves structured project favicon resolution causes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const root = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "starcode-asset-favicon-error-",
      });
