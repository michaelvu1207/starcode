...
export default Effect.gen(function* () {
  ...
  `;
  const hasLegacyPiRuns =
    existingColumns.length > 0 && !existingColumns.some((column) => column.name === "provider");

  if (hasLegacyPiRuns) {
    yield* sql`ALTER TABLE projection_agent_runs RENAME TO legacy_pi_projection_agent_runs`;
    // SQLite carries index names across a table rename. Release the legacy
    // name so the replacement table receives the current index below.
    yield* sql`DROP INDEX IF EXISTS idx_projection_agent_runs_parent_status`;
  }
  ...
  if (hasLegacyPiRuns) {
    // The baseline projection was exclusively fed by Pi lifecycle events, so
    // its previously implicit provider identity is unambiguous. Merge after
    // the event-log fold so rows that have no surviving event remain durable.
    yield* sql`
      INSERT INTO projection_agent_runs (
        parent_thread_id,
        provider,
      ...
      SELECT
        ...
        transcript_state,
        NULL
      FROM legacy_pi_projection_agent_runs
      WHERE true
      ON CONFLICT(parent_thread_id, provider, agent_run_id) DO UPDATE SET
        launch_tool_use_id = coalesce(excluded.launch_tool_use_id, launch_tool_use_id),
    ...
    `;
    yield* sql`DROP TABLE legacy_pi_projection_agent_runs`;
  }
