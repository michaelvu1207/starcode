...
const TelemetryEnvConfig = Config.all({
  posthogKey: Config.string("STARCODE_POSTHOG_KEY").pipe(
    Config.withDefault("phc_XOWci4oZP4VvLiEyrFqkFjP4CZn55mjYYBMREK5Wd6m"),
  ),
  posthogHost: Config.string("STARCODE_POSTHOG_HOST").pipe(
    Config.withDefault("https://us.i.posthog.com"),
  ),
  enabled: Config.boolean("STARCODE_TELEMETRY_ENABLED").pipe(Config.withDefault(true)),
  flushBatchSize: Config.number("STARCODE_TELEMETRY_FLUSH_BATCH_SIZE").pipe(Config.withDefault(20)),
  maxBufferedEvents: Config.number("STARCODE_TELEMETRY_MAX_BUFFERED_EVENTS").pipe(
...
>()("starcode/telemetry/AnalyticsService") {
    ...
    const payload = {
      ...
      batch: events.map((event) => ({
        ...
        properties: {
          ...
          starcodeVersion: packageJson.version,
          clientType,
        },
