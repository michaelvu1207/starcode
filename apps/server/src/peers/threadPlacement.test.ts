...
describe("resolveThreadModelSelection", () => {
  ...
  it("inherits provider options unless the caller explicitly replaces them", () => {
    const base = {
      ...selection("pi", "openai-codex/gpt-5.5"),
      options: [
    ...
    expect(
      resolveThreadModelSelection({
        locationDefault: base,
        overrides: { providerOptions: [{ id: "effort", value: "high" }] },
      }),
    ).toEqual({
      instanceId: "pi",
    ...
    ).toEqual({ instanceId: "pi", model: "openai-codex/gpt-5.5", options: [] });
  });
