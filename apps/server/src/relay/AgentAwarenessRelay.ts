...
import { projectThreadAwareness } from "@starcode/shared/agentAwareness";
...
>()("starcode/relay/AgentAwarenessRelay") {}
      ...
      switch (startupState) {
        case "waiting-for-link":
          yield* Effect.logInfo(
            "agent activity publishing standby; waiting for starcode Connect link reconciliation",
          );
          break;
        case "disabled":
          yield* Effect.logInfo(
            "agent activity publishing disabled by starcode Connect configuration",
          );
          break;
        case "enabled":
