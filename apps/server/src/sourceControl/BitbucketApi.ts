...
import { sanitizeBranchFragment } from "@starcode/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@starcode/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("STARCODE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("STARCODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("STARCODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("STARCODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
>()("starcode/sourceControl/BitbucketApi") {}
...
function checkoutBranchName(input: {
  ...
  return `starcode/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set STARCODE_BITBUCKET_EMAIL and STARCODE_BITBUCKET_API_TOKEN, or STARCODE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
