...
import { GitCommandError, SourceControlProviderError } from "@starcode/contracts";
...
const CLONE_URLS = {
  ...
  url: "https://github.com/octocat/starcode",
  sshUrl: "git@github.com:octocat/starcode.git",
};
  ...
  const serviceLayer = SourceControlRepositoryService.layer.pipe(
    ...
    Layer.provide(
      ServerConfig.layerTest(
        process.cwd(),
        input.fileSystem
          ? "/tmp/starcode-source-control-repos"
          : { prefix: "starcode-source-control-repos-" },
      ),
    ),
  );
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/starcode" }]);
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const parent = yield* fs.makeTempDirectoryScoped({
      prefix: "starcode-source-control-clone-parent-",
    });
    const destinationPath = `${parent}/starcode`;
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", CLONE_URLS.url, "starcode"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("preserves destination probe failures instead of treating them as missing paths", () => {
  const fileSystemCause = PlatformError.systemError({
    ...
    module: "FileSystem",
    method: "exists",
    pathOrDescriptor: "/restricted/starcode",
  });
  ...
  return Effect.gen(function* () {
    ...
    const error = yield* Effect.flip(
      service.cloneRepository({
        ...
        destinationPath: "/restricted/starcode",
      }),
    );
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/starcode", visibility: "private" },
    ]);
