...
const harness = (
  ...
  options: {
    readonly callerModelSelection?: ModelSelection | null;
  ...
  const callerModelSelection =
    options.callerModelSelection === undefined
      ? defaultModelSelection
      : options.callerModelSelection;
    ...
    getThreadShellById: (threadId: ThreadId) =>
      Effect.succeed(
        threadId === localId
          ...
          : threadId === callerId && callerModelSelection !== null
            ? Option.some({
                ...shell(callerId, "2026-07-30T09:30:00.000Z"),
                modelSelection: callerModelSelection,
              })
            : Option.none(),
      ),
...
it.effect("inherits the caller model when the destination project has no default", () => {
  const inherited: ModelSelection = {
    instanceId: ProviderInstanceId.make("pi"),
    model: "openai-codex/gpt-5.5",
  };
  const { layer, dispatched } = harness(Option.none(), {
    callerModelSelection: inherited,
...
it.effect("persists explicit provider options on the created thread command", () => {
  const inherited: ModelSelection = {
    instanceId: ProviderInstanceId.make("pi"),
    model: "openai-codex/gpt-5.5",
    options: [
  ...
  const { layer, dispatched } = harness(Option.none(), {
    projectDefaultModelSelection: inherited,
  });
  return Effect.gen(function* () {
    const service = yield* ThreadService;
    yield* service.createThread({
...
it.effect("does not spend the create allowance on invalid requests", () => {
  const { layer, dispatched } = harness(Option.none(), {
    callerModelSelection: null,
    projectDefaultModelSelection: null,
  });
  return Effect.gen(function* () {
    const service = yield* ThreadService;
    ...
    yield* service.createThread({
      ...
      message: "Use this explicit model",
      instanceId: "pi",
      model: "openai-codex/gpt-5.5",
    });
