...
> {
  ...
  readonly providerOptions?: ThreadCreateInput["providerOptions"] | undefined;
  ...
  const createThread: ThreadServiceShape["createThread"] = Effect.fn("ThreadService.createThread")(
    function* (options) {
      if (isLocalNode(options.node)) {
        ...
        const callerModelSelection = yield* projectionSnapshotQuery
          .getThreadShellById(options.callerThreadId)
          .pipe(
            Effect.map(
              Option.match({
                onNone: () => null,
        ...
        const modelSelection: ModelSelection | null = resolveThreadModelSelection({
          // A separate local task should be as easy to launch as an attached
          // agent. Project defaults still win when configured; an unconfigured
          // destination inherits the caller's known-good local provider/model.
          locationDefault: project.value.defaultModelSelection ?? callerModelSelection,
        ...
        // Charge the runaway backstop only after the request is valid. Invalid
        // placement/model attempts must not consume the caller's entire turn
        // allowance and make the corrected retry impossible.
