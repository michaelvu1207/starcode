...
export const LEGACY_HISTORY_READ_ONLY_DETAIL =
  "Claude Code and Codex terminal sessions remain available as read-only history. Their native resume cursors cannot be converted to Pi safely, so Starcode will not launch or silently replay them through Pi.";

export function legacyHistoryImportPolicy(_provider: HistoryProvider): "read-only" {
  return "read-only";
}
...
export const makeHistoryImporter = Effect.gen(function* () {
  ...
  const importSession = Effect.fn("history.import")(function* (input: HistoryImportInput) {
    ...
    // Pi is the only execution runtime. A Claude/Codex cursor names a native
    // store that Pi cannot resume, and translating it would produce a fresh
    // session that falsely appears to remember the imported conversation.
    // Existing import-registry rows were handled above so already-imported
    // threads remain navigable; only new executable imports are refused.
    if (legacyHistoryImportPolicy(entry.provider) === "read-only") {
      return yield* refuse("instance_driver_mismatch", LEGACY_HISTORY_READ_ONLY_DETAIL);
    }
