...
import { CodexSettings, ProviderInstanceId, TextGenerationError } from "@starcode/contracts";
...
const CodexTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "starcode-codex-text-generation-test-",
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "  cat > \"$output_path\" <<'__STARCODE_FAKE_CODEX_OUTPUT__'",
        input.output,
        "__STARCODE_FAKE_CODEX_OUTPUT__",
        "fi",
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "starcode-codex-text-" });
...
it.layer(CodexTextGenerationTestLayer)("CodexTextGeneration", (it) => {
  ...
  it.effect("uses STARCODE_CODEX_LAUNCH_ARGS for codex exec over settings", () =>
    withFakeCodexEnv(
      {
        output: JSON.stringify({
          subject: "Add important change",
        ...
        environment: { STARCODE_CODEX_LAUNCH_ARGS: " --strict-config --listen off " },
        requireArg: "--strict-config",
