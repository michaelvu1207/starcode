...
import { GrokSettings, ProviderInstanceId } from "@starcode/contracts";
...
const GrokTextGenerationTestLayer = ServerConfig.ServerConfig.layerTest(process.cwd(), {
  prefix: "starcode-grok-text-generation-test-",
...
function withFakeAcpGrok<A, E, R>(
  ...
  return Effect.gen(function* () {
    const tempDir = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "starcode-grok-text-acp-"));
...
it.layer(GrokTextGenerationTestLayer)("GrokTextGeneration", (it) => {
  it.effect("uses ACP with disabled tool capabilities and forwards the requested model id", () => {
    const requestLogDir = NodeFS.mkdtempSync(
      NodePath.join(NodeOS.tmpdir(), "starcode-grok-text-log-"),
    );
    ...
    return withFakeAcpGrok(
      {
        ...
        STARCODE_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
  ...
  it.effect("extracts the JSON object when Grok wraps it in conversational text", () =>
    withFakeAcpGrok(
      {
        STARCODE_ACP_PROMPT_RESPONSE_TEXT:
  ...
  it.effect("surfaces ACP request failures as text generation errors", () =>
    withFakeAcpGrok(
      {
        STARCODE_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({ branch: "unreachable" }),
      },
      (textGeneration) =>
        Effect.gen(function* () {
  ...
  it.effect("fails with TextGenerationError when output is unparseable JSON", () =>
    withFakeAcpGrok(
      {
        STARCODE_ACP_PROMPT_RESPONSE_TEXT: "totally not json output from a confused model",
      },
      (textGeneration) =>
        Effect.gen(function* () {
          const error = yield* Effect.flip(
