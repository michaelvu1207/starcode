...
import { GrokSettings } from "@starcode/contracts";
    ...
    Effect.gen(function* () {
      ...
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const dir = yield* fs.makeTempDirectoryScoped({ prefix: "starcode-grok-version-" });
    ...
    Effect.gen(function* () {
      const snapshot = yield* Effect.scoped(
        Effect.gen(function* () {
          const fs = yield* FileSystem.FileSystem;
          const path = yield* Path.Path;
          const dir = yield* fs.makeTempDirectoryScoped({ prefix: "starcode-grok-success-" });
          const grokPath = path.join(dir, "grok");
          yield* fs.writeFileString(
            grokPath,
