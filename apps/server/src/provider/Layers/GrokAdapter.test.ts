...
const grokAdapterTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "starcode-grok-adapter-test-",
...
it.layer(grokAdapterTestLayer)("GrokAdapterLive", (it) => {
  ...
  it.effect("reports a Grok session running only while the prompt is in flight", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          STARCODE_ACP_EMIT_TOOL_CALLS: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const requestOpened =
  ...
  it.effect("completes a Grok turn from xAI prompt completion when the prompt RPC hangs", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          STARCODE_ACP_EMIT_XAI_PROMPT_COMPLETE_THEN_HANG: "1",
          STARCODE_ACP_EMIT_FOREIGN_SESSION_UPDATES: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
  ...
  it.effect("does not report a synthetic stop reason when xAI omits one", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          STARCODE_ACP_EMIT_XAI_PROMPT_COMPLETE_THEN_HANG: "1",
          STARCODE_ACP_OMIT_XAI_PROMPT_COMPLETE_STOP_REASON: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
  ...
  it.effect("lets Stop unblock a fully silent Grok prompt and accept a follow-up turn", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          STARCODE_ACP_HANG_FIRST_PROMPT_FOREVER: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
  ...
  it.effect("does not let a cancelled prompt settlement consume the follow-up prompt slot", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          STARCODE_ACP_HANG_FIRST_PROMPT_FOREVER: "1",
          STARCODE_ACP_REQUEST_LOG_PATH: requestLogPath,
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
  ...
  it.effect("drops late ACP notifications after a turn is cancelled", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          STARCODE_ACP_HANG_PROMPT_FOREVER: "1",
          STARCODE_ACP_EMIT_LATE_UPDATE_AFTER_CANCEL: "1",
        }),
      );
  ...
  it.effect("ignores replayed session/load updates when resuming a Grok session", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          STARCODE_ACP_EMIT_LOAD_REPLAY: "1",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const runtimeEvents: ProviderRuntimeEvent[] = [];
    ...
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({
          STARCODE_ACP_REQUEST_LOG_PATH: requestLogPath,
          STARCODE_ACP_EMIT_TOOL_CALLS: "1",
          STARCODE_ACP_ALLOW_ONCE_OPTION_ID: "agent-defined-approval-id",
        }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const eventsFiber = yield* Stream.runForEach(adapter.streamEvents, (event) =>
  ...
  it.effect("handles xAI ask_user_question extension requests", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockGrokWrapper({ STARCODE_ACP_EMIT_XAI_ASK_USER_QUESTION: "1" }),
      );
      const adapter = yield* makeTestAdapter(wrapperPath);
      const requested =
