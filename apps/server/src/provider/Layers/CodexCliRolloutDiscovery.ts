...
import type { CodexCliSubagentInvocation } from "@starcode/shared/codexCliSubagent";
...
const PROMPT_FILE_MAX_BYTES = 2 * 1024 * 1024;
...
/**
 * Resolve the exact prompt used by a detected launch. A literal command-line
 * prompt is already available; for Claude's common `$(cat file)` form, read
 * only the proved literal file from the shell cwd and keep the same hard size
 * bound as rollout-head inspection.
 */
export async function resolveCodexCliInvocationPrompt(input: {
  readonly invocation: CodexCliSubagentInvocation;
  readonly parentCwd: string;
}): Promise<string | null> {
  if (input.invocation.prompt !== undefined) return input.invocation.prompt;
  if (input.invocation.promptFile === undefined) return null;

  const shellCwd = NodePath.resolve(input.parentCwd, input.invocation.shellCwd ?? ".");
  const promptPath = NodePath.resolve(shellCwd, input.invocation.promptFile);
  try {
    const stats = await NodeFSP.stat(promptPath);
    if (!stats.isFile() || stats.size > PROMPT_FILE_MAX_BYTES) return null;
    return await NodeFSP.readFile(promptPath, "utf8");
  } catch {
    return null;
  }
}
