...
export const makeCliUsageStore = (options: MakeCliUsageStoreOptions): CliUsageStoreShape => {
  ...
  const runRefresh = async (nowMs: number): Promise<void> => {
    await loadCache();
    const [files, codexPriorityTier] = await Promise.all([
...
export const layer: Layer.Layer<CliUsageStore, never, ServerConfig.ServerConfig> = Layer.effect(
  CliUsageStore,
  Effect.map(ServerConfig.ServerConfig, (config) =>
    CliUsageStore.of(makeCliUsageStore({ cachePath: config.cliUsageCachePath })),
  ),
);
