...
// @effect-diagnostics nodeBuiltinImport:off - node:os resolves the shared starcode home guard.
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start starcode once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.starcode database. Use an isolated --base-dir.";
  }
}
...
export const runSqliteState = Effect.fn("runSqliteState")(function* (
  ...
  const sharedHome = path.resolve(options.sharedHome ?? path.join(NodeOS.homedir(), ".starcode"));
...
export const starcodeSqliteStateCommand = Command.make(
  "starcode-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit starcode base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated starcode SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(starcodeSqliteStateCommand, { version: "0.0.0" }).pipe(
