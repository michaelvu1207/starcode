...
const PI_FAUX_API = "starcode-fleet-faux";
const PI_FAUX_MODEL = "starcode-faux/fleet-gate";
const PI_FAUX_PROVIDER = "starcode-faux";
const MODEL_SELECTION = { instanceId: "pi", model: PI_FAUX_MODEL } as const;
...
export class RealFleetGateDriver implements FleetGateDriver, AsyncDisposable {
  ...
  static async start(repoRoot = process.cwd()): Promise<RealFleetGateDriver> {
    ...
    const preloadPath = NodePath.join(
      resolvedRepoRoot,
      "apps",
      "server",
      "integration",
      "fleet",
      "fixtures",
      "pi-fleet-gate-provider.mjs",
    );
    const serverEnvironment: NodeJS.ProcessEnv = {
      ...process.env,
      NODE_OPTIONS: [
        process.env.NODE_OPTIONS?.trim(),
        `--import=${NodeURL.pathToFileURL(preloadPath).href}`,
      ]
    ...
    const harness = await ThreeNodeFleetHarness.make({
      ...
      launchNode: makeSourceServerNodeLauncher(resolvedRepoRoot, serverEnvironment),
    ...
    serverEnvironment.STARCODE_FLEET_MCP_CAPTURE_SOCKET = captureBroker.socketPath;
      ...
      const projects = Object.fromEntries(
        await Promise.all(
          FLEET_HARNESS_NODE_NAMES.map(async (name) => {
            ...
            const piAgentDir = NodePath.join(harness.nodes[name].homeDir, "pi-agent");
            await NodeFSP.mkdir(stateDir, { recursive: true });
            await NodeFSP.mkdir(workspace, { recursive: true });
            await NodeFSP.mkdir(piAgentDir, { recursive: true });
            await NodeFSP.writeFile(
              NodePath.join(piAgentDir, "models.json"),
              `${JSON.stringify(
                {
                  providers: {
                    [PI_FAUX_PROVIDER]: {
                      name: "Starcode Fleet Faux",
                      api: PI_FAUX_API,
                      baseUrl: "http://127.0.0.1:0",
                      apiKey: "fleet-gate-test-key",
                      models: [
                        {
                          id: "fleet-gate",
  ...
  readonly restartNode = async (node: FleetHarnessNodeName): Promise<void> => {
    await this.harness.restartNode(node);
    if (node === "alpha") {
      // Alpha's first faux-Pi turn is intentionally held until this restart.
      // Native recovery re-enters the provider and mints a fresh provider-
      // scoped MCP credential; adopting it here both proves that recovery ran
      // and keeps the public MCP assertions usable after the old token expires
      // with the stopped process.
      const endpoint = `${this.harness.nodes.alpha.baseUrl}/mcp`;
      const token = await this.#captureBroker.take(endpoint);
      const mcp = new RealMcpClient(endpoint, token);
      await mcp.initialize();
      this.#mcp = mcp;
    }
  };
  ...
  readonly bootstrapInstructions = async (node: FleetHarnessNodeName): Promise<string> => {
    const capturePath = NodePath.join(this.harness.nodes[node].homeDir, "fleet-bootstrap.txt");
    let captured: string | undefined;
    await eventually("Pi did not publish its effective system prompt", async () => {
      try {
        captured = await NodeFSP.readFile(capturePath, "utf8");
    ...
    if (captured === undefined) throw new Error("Pi system prompt capture was empty.");
    return captured;
  };
