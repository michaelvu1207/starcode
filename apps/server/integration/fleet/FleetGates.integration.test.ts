...
describeRealFleet("native Pi three-node fleet gates", () => {
  it("passes G0 Pi bootstrap and tool integrity", async () => {
    ...
    try {
      ...
      expect(bootstrap).toContain("Starcode's native Pi coding agent");
      expect(bootstrap).toContain("starcode_new_task");
      ...
      expect(bootstrap).not.toContain("codex app-server");
    } finally {
      await driver.dispose();
    }
  }, 600_000);

  it("passes G1 through the public MCP thread tools", async () => {
    const driver = await RealFleetGateDriver.start();
    try {
      expect(
  ...
  it("passes G3 and then G2 with only alpha-beta and beta-gamma pairing", async () => {
    ...
    try {
      ...
      const heldPiThreadId = "thread-master-alpha";
      expect(
        (await driver.readThread({ caller: "alpha", threadId: heldPiThreadId })).messages,
      ).not.toContain("Native Pi fleet gate response.");
      ...
      await expect
        .poll(
          async () =>
            (
              await driver.readThread({
                caller: "alpha",
                threadId: heldPiThreadId,
              })
            ).messages,
          { timeout: 20_000, interval: 100 },
        )
        .toContain("Native Pi fleet gate response.");
    } finally {
      await driver.dispose();
    }
  }, 600_000);
