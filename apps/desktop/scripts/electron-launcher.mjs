...
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["starcode-dev"] : ["starcode"];
...
// oxlint-disable-next-line starcode/no-global-process-runtime -- Standalone launcher script has no Effect runtime.
...
export function makeDevelopmentLauncherScript({
  ...
  const envEntries = [
    ...
    ["STARCODE_PORT", environment.STARCODE_PORT],
    ["STARCODE_HOME", environment.STARCODE_HOME],
    ["STARCODE_COMMIT_HASH", environment.STARCODE_COMMIT_HASH],
    ["STARCODE_OTLP_TRACES_URL", environment.STARCODE_OTLP_TRACES_URL],
    ["STARCODE_OTLP_EXPORT_INTERVAL_MS", environment.STARCODE_OTLP_EXPORT_INTERVAL_MS],
    ["STARCODE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} --starcode-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the starcode name.
