    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          ...
          STARCODE_DESKTOP_LAN_HOST: "192.168.1.50",
...
const withHarness = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    const baseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "starcode-desktop-backend-config-test-",
    });
...
describe("DesktopBackendConfiguration", () => {
  it.effect("resolvePrimary produces a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.STARCODE_PORT);
        assert.isUndefined(first.env.STARCODE_MODE);
        assert.isUndefined(first.env.STARCODE_DESKTOP_LAN_HOST);
        ...
        assert.equal(first.bootstrap.starcodeHome, environment.baseDir);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const linuxEntryPath = "/tmp/starcode code's launch/entry file.mjs";
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.equal(config.bootstrap.starcodeHome, environment.baseDir);
          assert.isTrue(Option.isNone(config.preflightFailure));
        }).pipe(
          Effect.provide(
            DesktopBackendConfiguration.layer.pipe(
  ...
  it("resolvePrimaryLabel is runSync-safe against the real WSL availability probe", async () => {
    ...
    // oxlint-disable-next-line starcode/no-manual-effect-runtime-in-tests -- This test intentionally replicates the sync IPC handler's runSync path to catch a regression to async-only resolution; it.effect would mask it.
    const runtime = ManagedRuntime.make(
      DesktopBackendConfiguration.layer.pipe(
        ...
        Layer.provideMerge(
          makeEnvironmentLayer("/tmp/starcode-wsl-isavailable", { platform: "win32" }),
        ),
    ...
    try {
      ...
      // oxlint-disable-next-line starcode/no-manual-effect-runtime-in-tests -- Same reason: this is the synchronous resolution the IPC handler performs.
