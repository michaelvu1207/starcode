...
vi.mock("electron", async (importOriginal) => ({
  ...
  session: {
    fromPartition: vi.fn(() => ({
      getUserAgent: vi.fn(() => "Mozilla/5.0 Electron/41.5.0 starcode/1.2.3"),
...
function makeTestLayer(input: {
  ...
  const desktopAppSettingsLayer = Layer.succeed(DesktopAppSettings.DesktopAppSettings, {
    ...
    setDiscordPresenceEnabled: () => Effect.die("unexpected Discord presence toggle"),
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        Layer.mock(PreviewManager.PreviewManager)({
          ...
          isBrowserPartition: (partition) => partition.startsWith("persist:starcode-preview-"),
          getBrowserPartition: () => Effect.succeed("persist:starcode-preview-test"),
        }),
      ),
    ),
  );
  ...
  it("recognizes only same-origin renderer navigations", () => {
    assert.isTrue(
      DesktopWindow.isSameOriginRendererNavigation({
        applicationUrl: "starcode://app/",
        navigationUrl: "starcode://app/settings/connections",
      }),
    );
    assert.isFalse(
      DesktopWindow.isSameOriginRendererNavigation({
  ...
  it.effect("does not open a development window until the backend is ready", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepEqual(fakeWindow.loadURL.mock.calls[0], ["starcode-dev://app/"]);
  ...
  it.effect("recovers when the development renderer is temporarily unreachable", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        didFailLoad({}, -9, "ERR_UNEXPECTED", "starcode-dev://app/", true);
  ...
  it("retries only transient failures for the development renderer", () => {
    assert.isTrue(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
        applicationUrl: "starcode-dev://app/",
        errorCode: -102,
        isMainFrame: true,
        validatedUrl: "starcode-dev://app/",
      }),
    );
    assert.isFalse(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
