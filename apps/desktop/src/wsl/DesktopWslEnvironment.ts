...
import { buildRemoteNodeEnvScript } from "@starcode/ssh/tunnel";
...
>()("@starcode/desktop/wsl/DesktopWslEnvironment") {}
...
export const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_PREBUILD_MISSING_EXIT_CODE
    ? "WSL support is missing from this starcode build: the packaged Linux node-pty binary was not included. Rebuild the Windows artifact with `--wsl-prebuild <path-to-linux-pty.node>` or install a build that includes WSL support."
    : null;
...
const marker = path.join(prebuildDir, "starcode-wsl-node-pty.json");
...
const NODE_PTY_BUILD_SCRIPT = (linuxServerDir: string) =>
  [
    ...
    `printf '{"arch":"%s","modules":"%s","nodePtyVersion":"%s"}\\n' "$arch" "$modules" "$node_pty_version" > "$prebuild_dir/starcode-wsl-node-pty.json"`,
