...
export interface CodexCliSubagentInvocation {
  ...
  /** Literal file read by an exact `$(cat <path>)` prompt substitution. */
...
function promptAndModel(words: ReadonlyArray<ParsedShellWord>): {
  ...
  // Claude commonly launches concurrent Codex workers with
  // `codex exec "$(cat specs/worker.md)"`. The shell expansion is not a
  // literal prompt, but the file path itself is provable and can be read by
  // the host-side rollout correlator from the same shell cwd.
  const promptFileMatch =
    !promptWord?.literal && promptWord?.value
      ? /^\$\(cat\s+(?:--\s+)?([^\s"'`$()]+)\s*\)$/.exec(promptWord.value)
      : null;
  const promptFile = promptFileMatch?.[1];
  return {
    ...(prompt ? { prompt } : {}),
...
export function detectCodexCliSubagent(
  ...
  return {
    ...
    ...(facts.promptFile && promptIsLinkable ? { promptFile: facts.promptFile } : {}),
    ...
    detached:
      ...
      hasDetachedOperator(command, match.words[0]?.start ?? 0),
  };
}
