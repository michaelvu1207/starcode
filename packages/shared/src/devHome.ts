/**
 * Resolves worktree-local development state so feature work never shares the
 * installed app's live StarCode database by accident.
 */
...
export const resolveWorktreeStarCodeHome = (
  cwd: string,
): Effect.Effect<string | undefined, never, FileSystem.FileSystem | Path.Path> =>
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".starcode");
  });
