...
echo "worker launched"`,
  ...
  it("treats Claude's background-tool flag as detached and retains a literal prompt file", () => {
    expect(
      detectCodexCliSubagent("Bash", {
        command:
          'cd /work/audit && codex exec --sandbox workspace-write "$(cat specs/ui-inventory.md)" > logs/ui.log 2>&1',
