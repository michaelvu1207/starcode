...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "starcode-ssh-askpass-test-" });
      ...
      assert.equal(env.STARCODE_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "starcode");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\starcode-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\starcode-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
