...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@starcode/contracts";
...
const PUBLISHABLE_STARCODE_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteStarcodeCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_STARCODE_VERSION_PATTERN.test(appVersion)) {
