...
/**
 * Which provider owns the conversation a Starcode thread fork inherited.
 *
 * This is deliberately separate from `HistoryProvider`. Terminal history is
 * still a decoder for the two legacy CLI stores on disk, while fork
 * provenance can name Pi's native transcript store. Claude and Codex remain
 * accepted here so provenance rows written by builds from before the Pi
 * cutover keep decoding.
 */
export const HistoryForkProvider = Schema.Literals(["claude", "codex", "pi"]);
export type HistoryForkProvider = typeof HistoryForkProvider.Type;
