...
}).annotate({ description: "Links an authenticated cloud user to a starcode environment." });
...
>()("@starcode/contracts/relay/RelayClientPrincipal") {}
...
>()("@starcode/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in starcode Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["starcode-mobile", "starcode-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "starcode-mobile" as const;
export const RelayWebClientId = "starcode-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "starcode Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking starcode environments, connecting authorized clients, and publishing agent activity.",
  );
