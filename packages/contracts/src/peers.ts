...
export const PeerThreadCreateInput = Schema.Struct({
  ...
  providerOptions: Schema.optional(
    ProviderOptionSelections.annotate({
      description:
        "Provider-specific reasoning and context selections on the peer. Omit to inherit the peer project options; pass an empty array to clear inherited options.",
    }),
  ),
...
export class PeerFederationError extends Schema.TaggedErrorClass<PeerFederationError>()(
  ...
  override get message(): string {
    ...
    return `Peer ${this.operation}${target} failed: ${this.reason}.${detail ? ` ${detail}` : ""}`;
  }
}
