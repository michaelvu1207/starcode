...
import {
  ...
  FleetPiAccountImportInput,
  FleetPiAccountImportResult,
  FleetExchangeInput,
  FleetExchangeResult,
  FleetReconcileInput,
...
export class EnvironmentFleetHttpApi extends HttpApiGroup.make("fleet")
  ...
  .add(
    HttpApiEndpoint.post("importAccounts", "/api/fleet/accounts/import", {
      headers: OptionalBearerHeaders,
      payload: FleetPiAccountImportInput,
      success: FleetPiAccountImportResult,
      error: EnvironmentPeerMutationErrors,
    }).middleware(EnvironmentAuthenticatedAuth),
  ) {}
